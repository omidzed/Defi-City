...
import { sevenDayTrend } from '../utils/data-types';
...
export const CoinsList = ({ filteredCoins, isDarkMode }: CoinsListProps) => {
	...
	return (
		<div className={` ${isDarkMode ? 'text-white bg-dark' : 'text-dark bg-white'} w-11/12 mx-auto`}>
			<table className='w-full sortable'>
				<thead className={`sticky top-0 z-10 ${isDarkMode ? 'bg-dark' : 'bg-white'}`}>
					<tr className='p-0 h-px bg-gray-200 no-sort double-border'>
						<th colSpan={14}></th>
					</tr>
					<tr>
						<th className={`${commonStyles} text-center sticky min-w-[24px] md:min-w-10 pl-2 md:pl-8 cursor-pointer`}>#</th>
						<th className={`${commonStyles} text-left sticky md:left-[24px] md:min-w-10 indicator-left cursor-pointer`}>Coin</th>
						<th className={`${commonStyles} text-right sticky md:left-[12px] md:min-w-[48px] indicator-left cursor-pointer`}>Price</th>
						<th className={`${commonStyles} text-right sticky left-[51px] md:left-[52px] min-w-[100px] 2lg:min-w-[150px] md:table-cell`}>24h</th>
				...
				<tbody className='divide-y min-w-full'>
					{filteredCoins.map(coin => (
						<tr key={uuid()}>
							<td>
								<div className={`${commonStyles} flex items-center gap-2 text-center sticky min-w-[24px] md:min-w-10 pl-2 md:pl-8 no-sort cursor-pointer`}>
									<StarComponent isDarkMode={isDarkMode} />
									{coin.market_cap_rank}
								</div>
							</td>
							<td>
								<div className={`${commonStyles} flex text-left items-center flex-nowrap`}>
									<div className='object-contain w-8 mr-2'>
										<img
											src={coin.image}
									...
									<div className='flex flex-col md:gap-1 md:flex-row xl:justify-start xl:items-center md:items-baseline'>
										...
										<span className='text-name font-semibold sticky whitespace-nowrap '>{coin.name}</span>
										<span className='font-thin text-symbol align-baseline md:justify-end'>{`${coin.symbol.toUpperCase()}`}</span>
									</div>
								</div>
							</td>
